...
public class OrcColumnarRowSplitReader extends OrcSplitReader<BaseRow> {
	...
	public OrcColumnarRowSplitReader(
		...
		super(
				conf,
				convertToOrcType(fullFieldNames, fullFieldTypes, partitionKeys),
				selectedFields,
				conjunctPredicates,
				batchSize,
				path,
	...
	private static TypeDescription convertToOrcType(
			String[] fullFieldNames,
			DataType[] fullFieldTypes,
			List<String> partitionKeys) {
		List<String> fullNameList = Arrays.asList(fullFieldNames);
		List<String> orcNames = fullNameList.stream()
				.filter(n -> !partitionKeys.contains(n)).collect(Collectors.toList());
		List<TypeDescription> fields = orcNames.stream()
				.mapToInt(fullNameList::indexOf)
				.mapToObj(i -> fullFieldTypes[i].getLogicalType())
				.map(OrcColumnarRowSplitReader::logicalTypeToOrcType)
				.collect(Collectors.toList());
		TypeDescription struct = TypeDescription.createStruct();
		for (int i = 0; i < orcNames.size(); i++) {
			struct.addField(orcNames.get(i), fields.get(i));
		}
		return struct;
	}
