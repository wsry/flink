...
public class NettyShuffleEnvironmentOptions {
	...
	public static final ConfigOption<Integer> NETWORK_BUFFERS_PER_OUTGOING_CHANNEL =
		key("taskmanager.network.memory.buffers-per-outgoing-channel")
			.defaultValue(-1)
			.withDescription("Number of network buffers to use for each outgoing channel (subpartition). If a negative " +
				"value is set, value of " + NETWORK_BUFFERS_PER_CHANNEL.key() + " will be used. If a non-negative value" +
				" is set, value of " + NETWORK_BUFFERS_PER_CHANNEL.key() + " will be ignored.");
	...
	public static final ConfigOption<Integer> NETWORK_BUFFERS_PER_INCOMING_CHANNEL =
		key("taskmanager.network.memory.buffers-per-incoming-channel")
			.defaultValue(-1)
			.withDescription("Number of network buffers to use for each incoming channel (input channel). If a negative " +
				"value is set, value of " + NETWORK_BUFFERS_PER_CHANNEL.key() + " will be used. If a non-negative value " +
				"is set, value of " + NETWORK_BUFFERS_PER_CHANNEL.key() + " will be ignored.");
