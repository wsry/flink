...
public class PartitionedFileWriter implements AutoCloseable {
    ...
    public void writeBuffer(Buffer target, int targetSubpartition) throws IOException {
        writeBuffers(
                Collections.singletonList(
                        new SortBuffer.BufferWithChannel(target, targetSubpartition)));
    }
     ...
     */
    public void writeBuffers(List<SortBuffer.BufferWithChannel> bufferWithChannels)
            throws IOException {
        checkState(!isFinished, "File writer is already finished.");
        checkState(!isClosed, "File writer is already closed.");
        ...
        for (int i = 0; i < bufferWithChannels.size(); i++) {
            SortBuffer.BufferWithChannel bufferWithChannel = bufferWithChannels.get(i);
        ...
        if (dataFileChannel.write(bufferWithHeaders) < expectedBytes) {
            BufferReaderWriterUtil.writeBuffers(dataFileChannel, bufferWithHeaders);
        }
    }
