...
public class Lz4BlockDecompressor implements BlockDecompressor {
	...
	public int decompress(ByteBuffer src, int srcOff, int srcLen, ByteBuffer dst, int dstOff)
		...
		if (dst.capacity() - prevDstOff < originalLen) {
			throw new InsufficientBufferException(String.format(
				"Buffer length too small: %d %d %d.", dst.capacity(), prevDstOff, originalLen));
		}

		if (src.limit() - prevSrcOff - HEADER_LENGTH < compressedLen) {
			throw new DataCorruptionException(String.format(
				"Source data is not integral for decompression: %d %d %d.", src.limit(), prevSrcOff, compressedLen));
		}
		...
		try {
			...
			if (compressedLen != compressedLen2) {
				throw new DataCorruptionException(String.format(
					"Input is corrupted, unexpected compressed length: %d %d.", compressedLen, compressedLen2));
			}
	...
	public int decompress(byte[] src, int srcOff, int srcLen, byte[] dst, int dstOff)
		...
		if (dst.length - dstOff < originalLen) {
			throw new InsufficientBufferException(String.format(
				"Buffer length too small: %d %d %d.", dst.length, dstOff, originalLen));
		}

		if (src.length - srcOff - HEADER_LENGTH < compressedLen) {
			throw new DataCorruptionException(String.format(
				"Source data is not integral for decompression: %d %d %d.", src.length, srcOff, compressedLen));
		}
		...
		try {
			...
			if (compressedLen != compressedLen2) {
				throw new DataCorruptionException(String.format(
					"Input is corrupted: %d %d.", compressedLen, compressedLen2));
			}
		}
		catch (LZ4Exception e) {
			throw new DataCorruptionException("Input is corrupted", e);
	...
	private void validateLength(int compressedLen, int originalLen) throws DataCorruptionException {
		if (originalLen < 0
			...
			|| (originalLen != 0 && compressedLen == 0)) {
			throw new DataCorruptionException(String.format(
				"Input is corrupted, invalid length: %d %d.", compressedLen, originalLen));
		}
	}
}
