...
public class NettyShuffleEnvironmentConfiguration {
	...
	public static NettyShuffleEnvironmentConfiguration fromConfiguration(
		...
		int maxNumberOfConnections = configuration.getInteger(NettyShuffleEnvironmentOptions.MAX_NUM_TCP_CONNECTIONS);
		if (maxNumberOfConnections <= 0) {
			maxNumberOfConnections = Math.max(1, nettyConfig != null ? nettyConfig.getNumberOfSlots() : 1);
		}
