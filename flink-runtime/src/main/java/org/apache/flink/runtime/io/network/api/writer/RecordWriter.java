...
public abstract class RecordWriter<T extends IOReadableWritable> implements AvailabilityProvider {
	...
	protected void emit(T record, int targetChannel) throws IOException, InterruptedException {
		...
		boolean shouldCopy = serializer.serializeRecord(record, flushAlways || randomTriggered);

		// Make sure we don't hold onto the large intermediate serialization buffer for too long
		if (shouldCopy && copyFromSerializerToTargetChannel(serializer, targetChannel)) {
			serializer.prune();
		}
	}
