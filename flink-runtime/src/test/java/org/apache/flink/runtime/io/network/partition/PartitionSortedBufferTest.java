...
public class PartitionSortedBufferTest {
	...
	public void testWriteAndReadSortBuffer() throws Exception {
		...
		while (true) {
			// record size may be larger than buffer size which means a record may span multiple segments
		...
		while (sortBuffer.hasRemaining()) {
			...
			SortBuffer.BufferWithChannel bufferAndChannelIndex = sortBuffer.copyData(readBuffer);
			int subpartition = bufferAndChannelIndex.getChannelIndex();
			buffersRead[subpartition].add(bufferAndChannelIndex.getBuffer());
			numBytesRead[subpartition] += bufferAndChannelIndex.getBuffer().readableBytes();
		}

		assertEquals(totalBytesWritten, sortBuffer.numBytes());
		checkWriteAndReadResult(numSubpartitions, numBytesWritten, numBytesRead, dataWritten, buffersRead);
	}

	public static void checkWriteAndReadResult(
	...
	public void testWriteMoreDataThanCapacity() throws Exception {
		...
		for (int i = 1; i < bufferPoolSize; ++i) {
			appendDataAndCheckResult(sortBuffer, bufferSize, true, bufferSize * i, i, true);
		}

		// append should fail for insufficient capacity
		int numRecords = bufferPoolSize - 1;
		appendDataAndCheckResult(sortBuffer, bufferSize, false, bufferSize * numRecords, numRecords, true);
	}
	...
	public void testWriteLargeRecord() throws Exception {
		...
		SortBuffer sortBuffer = createSortBuffer(bufferPoolSize, bufferSize, 1);
		// append should fail for insufficient capacity
		appendDataAndCheckResult(sortBuffer, bufferPoolSize * bufferSize, false, 0, 0, false);
	}

	private void appendDataAndCheckResult(
	...
	public void testReadUnfinishedSortBuffer() throws Exception {
		...
		sortBuffer.copyData(MemorySegmentFactory.allocateUnpooledSegment(bufferSize));
	}
	...
	public void testReleaseSortBuffer() throws Exception {
		...
		SortBuffer sortBuffer = new PartitionSortedBuffer(bufferPool, 1, bufferSize);
