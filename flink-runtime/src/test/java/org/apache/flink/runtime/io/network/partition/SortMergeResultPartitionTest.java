...
public class SortMergeResultPartitionTest {
	...
	public void testWriteAndRead() throws Exception {
		...
		SortMergeResultPartition partition = createSortMergedResultPartition(numSubpartitions, bufferPool);
		...
		for (int i = 0; i < numRecords; ++i) {
			...
			if (isBroadCast) {
				...
				for (int subpartition = 0; subpartition < numSubpartitions; ++subpartition) {
					recordDataWritten(record, dataWritten, subpartition, numBytesWritten, Buffer.DataType.DATA_BUFFER);
				}
			} else {
		...
		for (int subpartition = 0; subpartition < numSubpartitions; ++subpartition) {
			...
			recordDataWritten(record, dataWritten, subpartition, numBytesWritten, Buffer.DataType.EVENT_BUFFER);
		}
		...
		for (int i = 0; i < numSubpartitions; ++i) {
			...
			while (view.isAvailable(Integer.MAX_VALUE)) {
				...
				buffersRead[i].add(new NetworkBuffer(segment, (buf) -> {}, buffer.getDataType(), numBytes));
				buffer.recycleBuffer();
			}
			view.releaseAllResources();
		}

		PartitionSortedBufferTest.checkWriteAndReadResult(
	...
	public void testWriteLargeRecord() throws Exception {
		int numBuffers = 100;
		BufferPool bufferPool = globalPool.createBufferPool(numBuffers, numBuffers);
		SortMergeResultPartition partition = createSortMergedResultPartition(10, bufferPool);
		...
		ResultSubpartitionView view = partition.createSubpartitionView(0, new NoOpBufferAvailablityListener());
	...
	public void testFlush() throws Exception {
		...
		assertEquals(0, partition.getNumPartitionedFiles());

		partition.flush(0);
		assertEquals(0, bufferPool.bestEffortGetNumOfUsedBuffers());
		assertEquals(1, partition.getNumPartitionedFiles());
		...
		partition.flushAll();
		assertEquals(0, bufferPool.bestEffortGetNumOfUsedBuffers());
		assertEquals(2, partition.getNumPartitionedFiles());
	...
	private SortMergeResultPartition createSortMergedResultPartition(
