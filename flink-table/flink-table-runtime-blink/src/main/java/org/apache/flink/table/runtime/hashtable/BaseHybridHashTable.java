...
public abstract class BaseHybridHashTable implements MemorySegmentPool {
	...
	public BaseHybridHashTable(
		...
		this.totalNumBuffers = memManager.computeNumberOfPages(fraction);
		// some sanity checks first
		checkArgument(totalNumBuffers >= MIN_NUM_MEMORY_SEGMENTS);
		this.availableMemory = new ArrayList<>(this.totalNumBuffers);
		try {
			List<MemorySegment> allocates = new ArrayList<>();
			memManager.allocatePages(new MemoryManager.Builder(owner)
					.numberOfPages(totalNumBuffers).ofAllTypes().withOutput(allocates).build());
