...
import org.apache.flink.table.planner.plan.nodes.physical.batch.BatchPhysicalDynamicPartitionCollector;
...
import static org.apache.flink.table.api.config.OptimizerConfigOptions.TABLE_OPTIMIZER_DYNAMIC_PARTITION_PRUNING_ENABLED;
...
public abstract class DynamicPartitionPruningRuleBase extends RelRule<RelRule.Config> {
    ...
    public boolean doMatches(
        ...
        if (!ShortcutUtils.unwrapContext(join)
                .getTableConfig()
                .get(TABLE_OPTIMIZER_DYNAMIC_PARTITION_PRUNING_ENABLED)) {
            return false;
        }
        if (factScan.dppCollector() != null) {
            // rule applied
            return false;
        }
    ...
    protected BatchPhysicalTableSourceScan createNewTableSourceScan(
        ...
        final BatchPhysicalDynamicPartitionCollector dppSink =
                createDynamicPartitionSink(dimSide, dimPartitionFields);

        return factScan.copy(newTable, dppSink);
    }

    private BatchPhysicalDynamicPartitionCollector createDynamicPartitionSink(
        ...
        return new BatchPhysicalDynamicPartitionCollector(
