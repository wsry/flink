 ...
 * children nodes.
 *
 * <p>This rule extends from {@link FilterJoinRule.FilterIntoJoinRule}, and tries to push the filter
 * which field references are all from one side join condition to the another side.
 *
 * <p>such as:
...
public abstract class FlinkFilterJoinRule<C extends FlinkFilterJoinRule.Config> extends RelRule<C>
    ...
    public static final ConfigOption<Boolean> FLINKFILTERJOINRULE_ENABLED =
            ConfigOptions.key("table.optimizer.FlinkFilterJoinRule-enabled")
                    .booleanType()
                    .defaultValue(true)
                    .withDescription("");
    ...
    protected void perform(RelOptRuleCall call, Filter filter, Join join) {
        ...
        if (tableConfig.get(FLINKFILTERJOINRULE_ENABLED)) {
        ...
        for (RexNode filter : filtersToPush) {
            ...
            if (joinInfo.leftSet().contains(inputBits)) {
                final RexNode shiftedFilter =
                        remapFilter(joinInfo.leftKeys, joinInfo.rightKeys, filter);
                ...
                ImmutableIntList rightKeysWithOffset =
                        ImmutableIntList.copyOf(
                                joinInfo.rightKeys.stream()
                                        ...
                                        .collect(Collectors.toList()));
                final RexNode shiftedFilter =
                        remapFilter(rightKeysWithOffset, joinInfo.leftKeys, filter);
    ...
    private RexNode remapFilter(
            ImmutableIntList oldKeys, ImmutableIntList newKeys, RexNode filter) {
